...
impl<R: Read> PbzxReader<R> {
    pub fn new(mut reader: R) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        debug!("Found pbzx magic with compression: {:?}, block size: {}", 
    ...
    pub fn decompress_to<W: Write>(&mut self, output: &mut W) -> Result<(), Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            if (uncompressed_size & (1 << 24)) == 0 {
                debug!("End of chunks signaled (uncompressed_size: {:#x})", uncompressed_size);
                break;
            }
            ...
            debug!("Processing chunk {} - uncompressed: {}, compressed: {}", 
                   total_chunks + 1, actual_uncompressed_size, compressed_size);
            ...
            if is_compressed {
                match self.compression_algo {
                    ...
                    _ => {
                        return Err(format!("Unsupported compression algorithm: {:?}", 
                                         self.compression_algo).into());
                    }
                }
            } else {
        ...
        info!("Processed {} chunks, {} total bytes", total_chunks, total_bytes);
        Ok(())
    }
}